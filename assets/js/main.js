...
submit.addEventListener("click", () => {
  ...
  emailjs
    .send("service_lz30xkg", "template_nnoo10c", params)
    .then(function (res) {
...
clickBut.addEventListener("click", () => {
  ...
  if (
    ...
    timeSelect.value.trim().length > 0 &&
    humanCount.value.trim().length > 0 &&
    emailInput.value.trim().length > 0 &&
    goPhone.value.trim().length > 0 &&
    goDate.value.trim().length > 0
  ) {
    Swal.fire({
